...
require File.dirname(__FILE__) + "/src/a_star"
...
class Node < AStarNode
  ...
  def guess_distance(other)
    puts @connections[other]
    puts @connections.values.min
    connect_other = @connections[other].is_a?(String)? 1 : @connections[other]
    connect_other || @connections.values.min
  end
  ...
  def movement_cost(other)
    @connections[other].is_a?(String)? 1 : @connections[other]
  end
end
